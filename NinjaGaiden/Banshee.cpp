...
void Fishmen::Update(DWORD dt, vector<LPGAMEOBJECT>* listObject)
{
	...
	if (abs(x - xInit) >= FISHMEN_DX_LIMIT) // di d? kho?ng c? d?nh
	{
		if ((direction == -1 && !(simon->GetX() < x)) ||
			(direction == 1 && !(x < simon->GetX()))) // di v? hu?ng c?a simon mà dã vu?t simon thì m?i d?i hu?ng
		{
...
#pragma region Xu li va cham Brick
	...
	if (!isAttacking) // dang t?n công thì v?n cho tr?ng l?c kéo xu?ng
	{
		bool isCollisionDirectionX = false;
		for (UINT i = 0; i < coEventsResult.size(); i++) // không cho fishmen vu?t qua g?ch lo?i nh? theo tr?c x
		{
...
#pragma region Update Animation
	if (isAttacking)
	{
		...
		if (isRunning)
		{
			...
			if (sprite->GetCurrentFrame() == FISHMEN_ANI_ATTACK) // dang trong tr?ng thái di mà quay v? frame attack thì set thành frame di
